...
// Module-level cache removed - using ConfigCache instead for proper multi-tenant isolation
let clientsCache: ClientsData | null = null;
const CACHE_TTL = 60000;
let clientsCacheTimestamp = 0;

function isClientsCacheValid(): boolean {
  return Date.now() - clientsCacheTimestamp < CACHE_TTL;
}

function clearClientsCache(): void {
  clientsCache = null;
  clientsCacheTimestamp = 0;
}

// Export this for configCache.ts to call when invalidating (backwards compatibility)
export function clearBotConfigCache(): void {
  // No-op now - cache is in ConfigCache class
...
export function getBotConfig(clientId: string, botId: string): BotConfig | null {
  // No caching here - let ConfigCache handle it
  // This is a synchronous fallback for JSON-based bots only
...
export async function getBotConfigAsync(clientId: string, botId: string): Promise<BotConfig | null> {
  // No caching here - ConfigCache handles all caching
...
export function getBotConfigByBotId(botId: string): BotConfig | null {
  // No caching here - synchronous fallback for JSON bots only
  return loadBotFromJsonByBotId(botId);
}

export async function getBotConfigByBotIdAsync(botId: string): Promise<BotConfig | null> {
  // No caching here - ConfigCache handles it
